...
export default function ToursCatalog({ initialTours, initialFilters }: ToursCatalogProps) {
  ...
  const filteredTours = useMemo(() => {
    const filtered = initialTours.filter((tour) => {
      ...
      if (filters.typology && !tour.typologies.includes(filters.typology)) return false;
  ...
  return (
    <div className="container px-4 py-12">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-heading text-4xl text-neutral-900">Explore Our Tours</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
        <div className={`${sidebarOpen ? "block" : "hidden"} md:block`}>
          <TourFiltersSidebar filters={filters} onChange={setFilters} />
        </div>
