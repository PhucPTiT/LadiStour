    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute inset-x-4 bottom-4 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <Button asChild className="rounded-full bg-white px-4 py-2 text-sm text-neutral-900 hover:bg-white">
      ...
      <CardContent className="space-y-3 px-5 py-5">
        ...
        <h3 className="font-heading text-xl leading-tight text-neutral-900">{tour.title}</h3>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            {tour.salePrice ? (
              <p className="text-xs text-neutral-400 line-through">{formatPrice(tour.price)}</p>
            ) : null}
            <p className="text-lg font-semibold text-emerald-700">{formatPrice(displayPrice)}</p>
          </div>
        </div>
      </CardContent>
    </Card>
