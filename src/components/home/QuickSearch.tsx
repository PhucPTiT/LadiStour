...
export default function QuickSearch() {
  ...
  return (
    <section className="container relative z-10 -mt-14 px-4" id="quick-search" data-home-section>
      <div className="lux-shadow-3d rounded-[26px] border border-neutral-200/80 bg-white p-5 md:p-7">
        <p className="mb-4 text-sm font-semibold tracking-[0.16em] text-neutral-500 uppercase">Quick Search</p>
        <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          <Select value={destination || "all"} onValueChange={(value) => setDestination(value === "all" ? "" : value)}>
          ...
          <Select value={typology || "all"} onValueChange={(value) => setTypology(value === "all" ? "" : value)}>
            <SelectTrigger className="h-12 w-full rounded-xl border-neutral-300 bg-white px-3 text-sm text-neutral-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]">
          ...
          <Button asChild className="h-12 rounded-xl bg-emerald-700 px-6 text-sm text-white shadow-[0_10px_22px_rgba(15,122,92,0.35)] hover:bg-emerald-800">
            <Link href={`/tours${query.toString() ? `?${query.toString()}` : ""}`}>
